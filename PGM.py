...
class PGMImage:
    ...
    def __init__(self, pgm_filename):
        ...
        with open(pgm_filename, "rb") as pgm_file:
            ...
            i = 0
            self.pixels = array(
                [array([b for b in pgm_file.read(self.cols)]) for i in range(self.rows)]
            )
    ...
    def normalize(self):
        ...
        def normalize(_px):
            return int(((_px - lo) / (hi - lo)) * self.quantization)

        self.point_transform(normalize)
    ...
    def truncate(self):
        ...
        self.point_transform(truncate)

    def point_transform(self, update_fn):
        for i in range(self.rows):
            for j in range(self.cols):
                self.pixels[i][j] = update_fn(self.pixels[i][j])
    ...
    def save(self, pgm_filename, normalize=False):
        ...
        with open(pgm_filename, "wb") as f:
            lines = [
                ...
                b"".join([bytes([i]) for i in self.unrolled_pixels()]),
            ]
            f.writelines(lines)
    ...
    def unrolled_pixels(self) -> List[int]:
        ...
        return [int(b) for b in self.pixels.flatten()]
